...
removeToDoElement = function(dateString, int, box)
    ...
    awful.spawn
        .easy_async_with_shell( -- Read json file in as string called stdout
   
        [[cat /home/vahid/.config/awesome/widget/calendar/to-do/to-do-background/todo.json]],
        function(stdout)
            ...
            awful.spawn.with_shell('echo \"' .. jsonString ..
                                       '\" > /home/vahid/.config/awesome/widget/calendar/to-do/to-do-background/todo.json')
            ...
            if tabSize(_G["toDoTable"][dateString]) > 0 then
                ...
                for i = 1, tabSize(_G["toDoTable"][dateString]), 1 do
                    if _G["toDoTable"][dateString][i].status == 0 then
                        panelLayout:insert(1, box.create(dateString, i,
                                                         _G["toDoTable"][dateString][i]))
                        count = count + 1
                    end
                end
                if count == 0 then
