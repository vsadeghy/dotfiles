...
local bar = function(s)
    ...
    s.panel:struts{top = dpi(27)}

    function bar_toggle()
        if s.panel.visible == false then
            awful.screen.connect_for_each_screen(
                function(s) s.panel.visible = true end)
            awesome.emit_signal("bar:true")
        elseif s.panel.visible == true then
            awful.screen.connect_for_each_screen(
                function(s) s.panel.visible = false end)
    ...
    s.notificationCenterBar = require('widget.bar.notifications-bar')(
                                  colors.pink, 7)
    ...
    s.panel:setup{
        ...
        {layout = wibox.layout.fixed.horizontal, spacing = dpi(5), tags},
        {
            layout = wibox.layout.fixed.horizontal,
            spacing = dpi(5),
            s.vpn,
...
awesome.connect_signal("bar:toggle", function() bar_toggle() end)
