...
function markup.font(font, text)
    return '<span font="' .. tostring(font) .. '">' .. tostring(text) ..
               '</span>'
end
-- Set the foreground.
function markup.fg(color, text)
    return '<span foreground="' .. tostring(color) .. '">' .. tostring(text) ..
               '</span>'
end
-- Set the background.
function markup.bg(color, text)
    return '<span background="' .. tostring(color) .. '">' .. tostring(text) ..
               '</span>'
end
...
function widget.new(args)
    ...
    local widget_instance = {
        hide_without_description = (args.hide_without_description == nil) and
            widget.hide_without_description or args.hide_without_description,
        merge_duplicates = (args.merge_duplicates == nil) and
            widget.merge_duplicates or args.merge_duplicates,
    ...
    function widget_instance:_load_widget_settings()
        ...
        self.font = args.font or beautiful.hotkeys_font or
                        "Sauce Code Pro Bold 9"
        self.description_font = args.description_font or
                                    beautiful.hotkeys_description_font or
                                    "Sauce Code Pro 8"
        self.group_margin =
            args.group_margin or beautiful.hotkeys_group_margin or dpi(6)
    ...
    function widget_instance:_add_hotkey(key, data, target)
        ...
        if not target[group][index] then
            ...
            if self.merge_duplicates and joined_mods == target[group][index].mod then
                target[group][index].key =
                    target[group][index].key .. "/" .. new_key.key
        ...
        for _, keys in pairs(self._cached_awful_keys) do
            for _, params in pairs(keys) do
                if #params.keylist > 4 then
                    ...
                    local keygroup = gtable.find_first_key(self._keygroups,
                                                           function(_, v)
    ...
    function widget_instance:_import_awful_keys()
        if next(self._cached_awful_keys) then return end
        ...
        if self.merge_duplicates then self:_abbreviate_awful_keys() end
    end
    ...
    function widget_instance:_create_group_columns(column_layouts, group, keys,
                                                   s, wibox_height)
        ...
        for i, key in ipairs(keys) do
            joined_descriptions = joined_descriptions .. key.description ..
                                      (i ~= #keys and "\n" or "")
        end
        -- +1 for group label:
        ...
        if items_height > available_height_px then
            ...
            local available_height_items =
                (available_height_px - group_label_height * 2) / line_height
            for i = 1, #keys do
                table.insert(((i < available_height_items) and new_keys or
                                 overlap_leftovers), keys[i])
            end
            keys = new_keys
            table.insert(keys, {
        ...
        local function insert_keys(_keys, _add_new_column)
            ...
            for i, key in ipairs(_keys) do
                local length = string.len(key.key or '') +
                                   string.len(key.description or '')
                ...
                local rendered_hotkey = markup.font(self.font, modifiers ..
                                                        (key.key or "") ..
                                                        markup.fg(colors.purple,
                                                                  " : ")) ..
                ...
                joined_labels = joined_labels .. rendered_hotkey ..
                                    (i ~= #_keys and "\n" or "")
            end
            current_column.layout:add(wibox.widget.textbox(joined_labels))
            local max_width, _ =
                wibox.widget.textbox(max_label_content):get_preferred_size(s)
            max_width = max_width + self.group_margin
            if not current_column.max_width or max_width >
                current_column.max_width then
                current_column.max_width = max_width
            end
            -- +1 for group label:
            current_column.height_px = (current_column.height_px or 0) +
                                           gstring.linecount(joined_labels) *
                                           line_height + group_label_height
    ...
    function widget_instance:_create_wibox(s, available_groups,
        ...
        for _, group in ipairs(available_groups) do
            local keys = gtable.join(show_awesome_keys and
                                         self._cached_awful_keys[group] or nil,
                                     self._additional_hotkeys[group])
            if #keys > 0 then
                self:_create_group_columns(column_layouts, group, keys, s,
                                           wibox_height)
            end
        end
        ...
        for _, item in ipairs(column_layouts) do
            if item.max_width > available_width_px then
                ...
                item.layout:insert(1, self:_group_label("PgUp - Prev Page",
                                                        self.label_bg))
            else
        ...
        function widget_obj.show(_self) _self.popup.visible = true end
    ...
    function widget_instance:show_help(c, s, show_args)
        ...
        for group, _ in pairs(self._group_list) do
            ...
            for group_name, data in pairs(self.group_rules) do
                if group_name == group and
                    (data.rule or data.rule_any or data.except or
                        data.except_any) then
        ...
        if not self._cached_wiboxes[s] then self._cached_wiboxes[s] = {} end
        ...
        help_wibox.keygrabber = awful.keygrabber.run(
...
function widget.show_help(...) return get_default_widget():show_help(...) end

--- Add hotkey descriptions for third-party applications
function widget.add_hotkeys(...) return get_default_widget():add_hotkeys(...) end
