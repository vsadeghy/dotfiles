...
local globalKeys = awful.util.table.join(
    ...
    end, {description = "go back", group = "Tiling"}), -- Default programs
    ...
    end, {description = "Open Atom", group = "Default programs"}),

                       awful.key({modkey}, "b", function()
        awful.spawn(apps.default.browser)
    end, {
        description = "Open Default browser (Brave)",
    ...
    -- 	),
    awful.key({modkey}, "d", function() awful.spawn(apps.default.chat) end,
              {description = "Open Discord", group = "Default programs"}),

                       awful.key({modkey}, "n", function()
        awful.spawn(apps.default.file_manager)
    end, {
        description = "Open File Manager (pcmanfm)",
        group = "Default programs"
        ...
        naughty.notify({
            ...
            text = awful.layout
                .getname(awful.layout.get(awful.screen.focused()))
        })
    ...
    end, {description = "restore minimized", group = "Tiling"}),

    -- Run DMenu Prompt
                       awful.key({modkey}, "r", function()
        awful.util.spawn("rofi -no-lazy-grab -show drun -theme dmenu.rasi")
    end, {description = "run dmenu prompt", group = "Launchers"}),

    -- Run DMenu Config Prompt
                       awful.key({modkey}, "c", function()
        awful.spawn("./.config/dmenu/dmenu-edit-configs.sh")
    end, {description = "run dmenu configs prompt", group = "Launchers"}),

    -- Run DMenu VPN Prompt
                       awful.key({modkey}, "v", function()
        awful.util.spawn("/home/vahid/.config/dmenu/nord-countries.sh")
    end, {description = "run dmenu vpn prompt", group = "Launchers"}),

    -- Run DMenu Projects Prompt
                       awful.key({modkey}, "p", function()
        awful.util.spawn("/home/vahid/.config/dmenu/projectFoldersRofi.sh")
    end, {description = "run dmenu projects prompt", group = "Launchers"}),

    -- Run Rofi Prompt
                       awful.key({modkey, "Shift"}, "r", function()
        awful.spawn.with_shell(
            'rofi -no-lazy-grab -show drun -theme centered.rasi')
    end, {description = "run rofi prompt", group = "Launchers"}),

    -- Run Rofi Prompt
                       awful.key({modkey, "Shift"}, "w", function()
        awful.spawn.with_shell(
            'rofi -no-lazy-grab -show window -theme window.rasi')
    end, {description = "run rofi window prompt", group = "Launchers"}),
    ...
    end, {description = 'toggle calendar', group = 'Awesome'}),
